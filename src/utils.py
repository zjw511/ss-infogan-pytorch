...
def plot_generated_data(config, generator, z, epoch, idx_c_d, idx_c_c):
    ...
    title = 'Fixed_%s_E-%s_Cd-%s_Cc-%s'%(config.model_name,epoch+1,idx_c_d,idx_c_c)
    # plt.figure(figsize=(10, 10))
    # plt.title(title, fontsize=25)
    # plt.xticks([])
    # plt.yticks([])
    # plt.xlabel('Continuous Code Index = %s'%idx_c_c, fontsize=20)
    # plt.ylabel('Discrete Code Index = %s'%idx_c_d, fontsize=20)
    ...
    # plt.close('all')
    torchvision.utils.save_image(gen_data,  os.path.join(result_dir, title+'onlyfig.png'), nrow=10 , normalize=True)
...
def plot_generated_data_only_cont(config, generator, z, epoch):
    with torch.no_grad():
        gen_data = generator(z).detach().cpu()
    title = 'Fixed_%s_E-%s'%(config.model_name,epoch+1)
    plt.figure(figsize=(config.dim_c_cont, 10))
    plt.title(title, fontsize=10)
    plt.xticks([])
    plt.yticks([])
    plt.xlabel('Continuous Code Range : [-1,1]', fontsize=8)
    plt.ylabel('Idx_Fixed_CC = Row_Index', fontsize=8)
    plt.imshow(np.transpose(vutils.make_grid(
        gen_data, nrow=10, padding=2, normalize=True), (1, 2, 0)))
    result_dir = os.path.join(
        config.project_root, 'results', config.model_name, 'images')
...
def save_config(config):
    ...
    save_dir = os.path.join(
        config.project_root, 'results/%s'%config.model_name)
    os.makedirs(save_dir, exist_ok=True)

    with open('%s/config.json'%save_dir, 'w') as fp:
